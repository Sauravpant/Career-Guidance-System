...
export const getProjectsByPhase = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user.id;
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, projects, "Phase projects fetched successfully"));
});
...
export const createProject = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!title || !description || !type) {
    throw new AppError(400, "Title, description, and type (GLOBAL/PHASE) are required");
  }
...
export const updateProject = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (steps !== undefined) {
    updateData.steps = Array.isArray(steps) ? steps.join("\n") : String(steps || "");
  }
